...
export default function DetailPanel() {
  ...
  const l2Summary = l2Set.size > 0 ? [...l2Set].map(t => L2_TYPES[t]?.name || t).join('、') : '无';
  ...
  return (
    <div style={{ position: 'absolute', right: 0, top: 44, width: 280, height: 'calc(100% - 44px)', background: 'rgba(13, 26, 45, 0.95)', borderLeft: '1px solid #333', padding: 16, overflowY: 'auto', zIndex: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ color: '#ffd700', fontSize: 18, fontWeight: 700, marginBottom: 4 }}>{modelDef?.name || `位置 (${coord.x}, ${coord.y})`}</div>
        <button onClick={() => setSelectedCellKey(null)} style={{ background: 'none', border: '1px solid #555', color: '#aaa', borderRadius: 4, padding: '2px 8px', cursor: 'pointer', fontSize: 14 }} title="关闭">✕</button>
      </div>
      <div style={{ color: '#888', fontSize: 12, marginBottom: 12 }}>坐标 ({coord.x}, {coord.y}) · {l1Name} · {l2Summary}</div>
      ...
      {entities.length === 0 && <div style={{ fontSize: 11, color: '#555', fontStyle: 'italic', marginTop: 8 }}>暂无关联数据。Phase 2 将支持添加人物和事件关联。</div>}
    </div>
  );
}
...
function Section({ icon, title, items }: { icon: string; title: string; items: { id: string; name: string }[] }) {
  ...
  return <div style={{ marginBottom: 12 }}><div style={{ color: '#4a90d9', fontSize: 13, fontWeight: 600, marginBottom: 4 }}>{icon} {title}</div>{items.map(e => <div key={e.id} style={{ fontSize: 12, color: '#aaa', padding: '2px 0' }}>{e.name}</div>)}</div>;
}
